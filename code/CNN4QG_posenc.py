...
class SolNet(nn.Module):
    def __init__(self, flow_net, hidden_dim=256, use_pos_encoding=True, num_frequencies=6):
        ...
        self.predictor = nn.Sequential(
            ...
            # nn.Linear(8, 4)  # output: cos x, sin x, cos y, sin y
            nn.Linear(32, out_dim)  # output: dx, dy
        )
...
for ep in range(1, epochs+1):
    ...
    for u1_initial, u2_initial, u1_next, u2_next in train_loader:
        ...
        u1_pred, u2_pred, u2_decoded = solnet(u1_initial, u2_initial)
    ...
    if ep % 10 == 0:
        ...
        print(f"ep {ep} time {end_time - start_time:.4f} | "
              f"train_u1: {train_loss_forecast_u1:.4f}  train_u2: {train_loss_forecast_u2:.4f} train_ae: {train_loss_ae:.4f} | "
