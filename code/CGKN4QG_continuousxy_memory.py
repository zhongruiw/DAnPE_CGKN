...
class CGN(nn.Module):
    def __init__(self, dim_u1, dim_z):
        ...
        # Homogeneous Tracers
        self.input_size = 4 # 2-step memory
        self.output_size = 2 + 2*dim_z
...
class CGKN(nn.Module):
    ...
    def forward(self, u_extended):
        ...
        dim_u1 = self.cgn.dim_u1 * 2 # 2-step memory
...
for ep in range(1, epochs+1):
    ...
    for iens in range(Ntrain):
        u1_initial = torch.cat([train_u1[iens, :-2], train_u1[iens, 1:-1]], dim=1) # shape (49, 256*2)
        u1_next = train_u1[iens, 2:] # shape (49, 256)
        u2_initial = train_u2[iens, 1:-1] # shape (49, 128, 128, 2)
        u2_next = train_u2[iens, 2:] # shape (49, 128, 128, 2)
...
with torch.no_grad():
    for iens in range(0, Ntrain):
        batch_u1 = torch.cat([train_u1[iens, :-2], train_u1[iens, 1:-1]], dim=1) # shape (49, 256*2)
        batch_u2 = train_u2[iens, 1:-1] # shape (49, 128, 128, 2)
...
sigma_hat[:dim_u1] = torch.sqrt(torch.mean((train_u1[:, 2:] - train_u1_pred)**2, dim=(0,1))).to("cpu")
...
def CGFilter(cgkn, sigma, u1, mu0, R0):
    ...
    for n in range(Nt):
        u1_memory = torch.cat([u1[:-2], train_u1[iens, 1:-1]], dim=1) # shape (49, 256*2)
...
for ep in range(epochs):
    for iens in range(Ntrain):
        ...
        u2_long = train_u2[iens, 2:].to(device)
