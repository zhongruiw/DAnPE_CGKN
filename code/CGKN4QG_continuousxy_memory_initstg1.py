...
u1 = torch.tensor(pos, dtype=torch.float) # shape (Nens, Nt, L, 2), keep tracers parallel
...
for ep in range(1, epochs+1):
    ...
    for iens in range(Ntrain):
        # u1_initial = torch.cat([train_u1[iens, n:-len_mem+n] for n in range(len_mem)], dim=1) # shape (49, L, 2*len_mem); not including u1[n], the forecast step
        # u1_next = train_u1[iens, len_mem:] # shape (49, 256)
        # u2_initial = train_u2[iens, len_mem-1:-1] # shape (49, 128, 128, 2)
        # u2_next = train_u2[iens, len_mem:] # shape (49, 128, 128, 2)

        u1_initial = torch.cat([train_u1[iens, n:Ntime-len_mem+n+1] for n in range(len_mem)], dim=2) # shape (50, L, 2*len_mem); including u1[n], the forecast step
        u1_next = train_u1[iens, len_mem-1:].view(Ntime-len_mem+1, dim_u1) # shape (50, L*2)
        u2_initial = train_u2[iens, len_mem-2:-1] # shape (50, 128, 128, 2)
        u2_next = train_u2[iens, len_mem-1:] # shape (50, 128, 128, 2)
...
torch.save(cgkn, r"../model/QG(Noisy)_sigmaxy01_continuousxy_CGKN_dimz256_memory_stage1.pt")
np.save(r"../model/QG(Noisy)_sigmaxy01_continuousxy_CGKN_dimz256_memory_train_loss_forecast_u1_history_stage1.npy", train_loss_forecast_u1_history)
np.save(r"../model/QG(Noisy)_sigmaxy01_continuousxy_CGKN_dimz256_memory_train_loss_forecast_u2_history_stage1.npy", train_loss_forecast_u2_history)
np.save(r"../model/QG(Noisy)_sigmaxy01_continuousxy_CGKN_dimz256_memory_train_loss_ae_history_stage1.npy", train_loss_ae_history)
np.save(r"../model/QG(Noisy)_sigmaxy01_continuousxy_CGKN_dimz256_memory_train_loss_forecast_z_history_stage1.npy", train_loss_forecast_z_history)

"""
cgkn = torch.load(r"../model/QG(Noisy)_sigmaxy01_continuousxy_CGKN_dimz256_memory_stage1.pt").to(device)
...
with torch.no_grad():
    for iens in range(0, Ntrain):
        # batch_u1 = torch.cat([train_u1[iens, n:-len_mem+n] for n in range(len_mem)], dim=1) # shape (49, 256*2)
        # batch_u2 = train_u2[iens, len_mem-1:-1] # shape (49, 128, 128, 2)
        batch_u1 = torch.cat([train_u1[iens, n:Ntime-len_mem+n+1] for n in range(len_mem)], dim=2) # shape (50, L, 2*len_mem)
        batch_u2 = train_u2[iens, len_mem-2:-1] # shape (50, 128, 128, 2)
        batch_z_flat = cgkn.autoencoder.encoder(batch_u2).view(batch_u2.size(0), dim_z)
...
sigma_hat = torch.zeros(dim_u1 + dim_z)
# sigma_hat[:dim_u1] = torch.sqrt(torch.mean((train_u1[:, len_mem:] - train_u1_pred)**2, dim=(0,1))).to("cpu")
sigma_hat[:dim_u1] = torch.sqrt(torch.mean((train_u1[:, len_mem-1:].view(Ntrain, Ntime-len_mem+1, -1)  - train_u1_pred)**2, dim=(0,1))).to("cpu")
...
for ep in range(epochs):
    for iens in range(Ntrain):
        ...
        # u1_initial = torch.cat([train_u1[iens, n:-len_mem+n] for n in range(len_mem)], dim=1) # shape (49, 256*2); not including u1[n], the forecast step
        # u1_next = train_u1[iens, len_mem:] # shape (49, 256)
        # u2_initial = train_u2[iens, len_mem-1:-1] # shape (49, 128, 128, 2)
        # u2_next = train_u2[iens, len_mem:] # shape (49, 128, 128, 2)
...
# torch.save(cgkn, r"../model/QG(Noisy)_sigmaxy01_continuousxy_CGKN_dimz256_memory_initstg1_stage2.pt")
# np.save(r"../model/QG(Noisy)_sigmaxy01_continuousxy_CGKN_dimz256_memory_initstg1_train_loss_forecast_u1_history_stage2.npy", train_loss_forecast_u1_history)
# np.save(r"../model/QG(Noisy)_sigmaxy01_continuousxy_CGKN_dimz256_memory_initstg1_train_loss_forecast_u2_history_stage2.npy", train_loss_forecast_u2_history)
# np.save(r"../model/QG(Noisy)_sigmaxy01_continuousxy_CGKN_dimz256_memory_initstg1_train_loss_ae_history_stage2.npy", train_loss_ae_history)
# np.save(r"../model/QG(Noisy)_sigmaxy01_continuousxy_CGKN_dimz256_memory_initstg1_train_loss_forecast_z_history_stage2.npy", train_loss_forecast_z_history)
# np.save(r"../model/QG(Noisy)_sigmaxy01_continuousxy_CGKN_dimz256_memory_initstg1_train_loss_da_history_stage2.npy", train_loss_da_history)


cgkn = torch.load(r"../model/QG(Noisy)_sigmaxy01_continuousxy_CGKN_dimz256_memory_initstg1_stage2.pt").to(device)
...
with torch.no_grad():
    for iens in range(Ntest):
        ...
        # batch_u2 = test_u2[iens, len_mem-1:-1] # shape (49, 128, 128, 2)
        batch_u1 = torch.cat([test_u1[iens, n:Ntime-len_mem+n+1] for n in range(len_mem)], dim=2) # shape (50, L, 2*len_mem)
        batch_u2 = test_u2[iens, len_mem-2:-1] # shape (50, 128, 128, 2)
        batch_z_flat = cgkn.autoencoder.encoder(batch_u2).view(batch_u2.size(0), dim_z)
        batch_u1_pred, batch_z_pred_flat  = cgkn(batch_u1, batch_z_flat)
        batch_z_pred = batch_z_pred_flat.view(batch_z_pred_flat.size(0), int(dim_z**0.5), int(dim_z**0.5))
        batch_u2_pred = cgkn.autoencoder.decoder(batch_z_pred)
...
np.save(r"../data/CGKN_sigmaxy01_continuousxy_memory_initstg1_xy_unit_OneStepPrediction.npy", test_u1_pred.to("cpu"))
np.save(r"../data/CGKN_sigmaxy01_continuousxy_memory_initstg1_psi_OneStepPrediction.npy", test_u2_pred.to("cpu"))
...
test_u1 = test_u1.to("cpu")
test_u2 = test_u2.to("cpu")
np.save(r"../data/CGKN_sigmaxy01_continuousxy_memory_initstg1_psi_DA.npy", test_mu_pred.to("cpu"))
