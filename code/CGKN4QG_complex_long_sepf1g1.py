...
for ep in range(1, epochs+1):
    ...
    if ep % 10 == 0:
        ...
        if val_loss_total < best_val_loss:
            ...
            torch.save(checkpoint, r"../model/CGKN_L1024_complex_long_sepf1g1_stage1.pt")
            status = "✅"
        else:
            status = ""
...
# torch.save(sigma_hat, "../data/CGKN_L1024_complex_long_sepf1g1_sigma_hat.pt")
    ...
    if itr % val_per_itrs == 0:
        ...
        with torch.no_grad():
            ...
            if val_loss_total < best_val_loss:
                ...
                torch.save(checkpoint, "../model/CGKN_L1024_complex_long_sepf1g1_stage2.pt")
                status = "✅"
            else:
                status = ""
...
np.save(r"../data/CGKN_L1024_complex_long_sepf1g1_xy_unit_OneStepPrediction.npy", test_u1_pred.to("cpu"))
np.save(r"../data/CGKN_L1024_complex_long_sepf1g1_psi_OneStepPrediction.npy", test_u2_pred.to("cpu"))
...
test_u1 = test_u1.to("cpu")
test_u2 = test_u2.to("cpu")
np.save(r"../data/CGKN_L1024_complex_long_sepf1g1_psi_DA.npy", test_mu_pred.to("cpu"))
...
np.save(r"../data/CGKN_L1024_complex_long_sepf1g1_psi_DA_std.npy", test_mu_std_pred)