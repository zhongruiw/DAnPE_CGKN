...
class CGN(nn.Module):
    def __init__(self, dim_u1, dim_z, use_pos_encoding=True, num_frequencies=6):
        ...
        in_dim = 2  # (x1, x2)
        self.in_dim = in_dim
        self.out_dim = in_dim*2 + in_dim*dim_z
        if use_pos_encoding:
    ...
    def forward(self, x): # x shape(Nt, L, 2)
        ...
        g1_nn = out[:, :, self.in_dim*2:].reshape(Nt, L*4, self.dim_z//2)
        g1_zero = torch.zeros(Nt, L*4, self.dim_z//2, device=x.device, dtype=out.dtype)
        g1 = torch.cat([g1_nn, g1_zero], dim=-1)                  # (Nt, L*4, dim_z)
...
torch.save(cgkn, r"../model/QG(Noisy)_64x64_sigmaxy01_CGKN_dimz256_g1z1_stage1.pt")
np.save(r"../model/QG(Noisy)_64x64_sigmaxy01_CGKN_dimz256_g1z1_train_loss_forecast_u1_history_stage1.npy", train_loss_forecast_u1_history)
np.save(r"../model/QG(Noisy)_64x64_sigmaxy01_CGKN_dimz256_g1z1_train_loss_forecast_u2_history_stage1.npy", train_loss_forecast_u2_history)
np.save(r"../model/QG(Noisy)_64x64_sigmaxy01_CGKN_dimz256_g1z1_train_loss_ae_history_stage1.npy", train_loss_ae_history)
np.save(r"../model/QG(Noisy)_64x64_sigmaxy01_CGKN_dimz256_g1z1_train_loss_forecast_z_history_stage1.npy", train_loss_forecast_z_history)
# np.save(r"../model/QG(Noisy)_64x64_sigmaxy01_CGKN_dimz256_train_loss_physics_history_stage1.npy", train_loss_physics_history)

"""
cgkn = torch.load(r"../model/QG(Noisy)_64x64_sigmaxy01_CGKN_dimz256_g1z1_stage1.pt").to(device)
...
with torch.no_grad():
    for i in range(0, Ntest, batch_size):
        batch_u1 = test_u1[i: i+batch_size]
        batch_u2 = test_u2[i: i+batch_size]
        batch_z_flat = cgkn.autoencoder.encoder(batch_u2).view(batch_u2.size(0), dim_z)
        batch_u1_pred, batch_z_flat_pred = cgkn(batch_u1, batch_z_flat)
        batch_z_pred = batch_z_flat_pred.view(-1, 2, z_h, z_w)
...
np.save(r"../data/CGKN_g1z1_64x64_sigmaxy01_xy_unit_OneStepPrediction_stage1.npy", test_u1_pred.to("cpu"))
np.save(r"../data/CGKN_g1z1_64x64_sigmaxy01_psi_OneStepPrediction_stage1.npy", test_u2_pred.to("cpu"))
...
torch.save(cgkn, r"../model/QG(Noisy)_64x64_sigmaxy01_CGKN_dimz256_g1z1_batchda_stage2.pt")
np.save(r"../model/QG(Noisy)_64x64_sigmaxy01_CGKN_dimz256_g1z1_batchda_train_loss_forecast_u1_history_stage2.npy", train_loss_forecast_u1_history)
np.save(r"../model/QG(Noisy)_64x64_sigmaxy01_CGKN_dimz256_g1z1_batchda_train_loss_forecast_u2_history_stage2.npy", train_loss_forecast_u2_history)
np.save(r"../model/QG(Noisy)_64x64_sigmaxy01_CGKN_dimz256_g1z1_batchda_train_loss_ae_history_stage2.npy", train_loss_ae_history)
np.save(r"../model/QG(Noisy)_64x64_sigmaxy01_CGKN_dimz256_g1z1_batchda_train_loss_forecast_z_history_stage2.npy", train_loss_forecast_z_history)
np.save(r"../model/QG(Noisy)_64x64_sigmaxy01_CGKN_dimz256_g1z1_batchda_train_loss_da_history_stage2.npy", train_loss_da_history)
...
# cgkn = torch.load(r"../model/QG(Noisy)_64x64_sigmaxy01_CGKN_dimz256_g1z1_batchda_stage2.pt").to(device)
...
np.save(r"../data/CGKN_64x64_g1z1_batchda_sigmaxy01_xy_unit_OneStepPrediction.npy", test_u1_pred.to("cpu"))
np.save(r"../data/CGKN_64x64_g1z1_batchda_sigmaxy01_psi_OneStepPrediction.npy", test_u2_pred.to("cpu"))
...
test_u1 = test_u1.to("cpu")
test_u2 = test_u2.to("cpu")
np.save(r"../data/CGKN_64x64_g1z1_batchda_sigmaxy01_psi_DA.npy", test_mu_pred.to("cpu"))
