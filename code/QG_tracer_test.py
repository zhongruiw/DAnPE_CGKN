...
if __name__ == "__main__": 
    ...
    L = 1 # Number of tracers
    sigma_xy = 0.1 # Tracer observation noise std (in sde)
    dt_ob = 2e-3 # Observation time interval
    ...
    psi_k_t1, x1, y1, qp_t1 = model.forward_ens(ens=1, Nt=Nt_obs, dt=dt, qp_ens=qp_t0, L=L, x0=x0[None, :], y0=y0[None, :])
    print(psi_k_t1.shape, x1.shape)
    psi_k_t = psi_k_t1.squeeze()
    # x_t = x1.squeeze()
    # y_t = y1.squeeze()
    ...
    np.savez('../data/qg_data_test.npz', **save)
