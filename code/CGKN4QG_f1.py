...
class CGN(nn.Module):
    ...
    def forward(self, x): # x shape(Nt, L, 2)
        Nt, L = x.shape[:2]
        unitx = torch.stack([torch.cos(x[:, :, 0]), torch.sin(x[:, :, 0]), torch.cos(x[:, :, 1]), torch.sin(x[:, :, 1])], dim=-1) # (Nt, L, 4)
        if self.use_pos_encoding:
            x = self.positional_encoding(x.view(-1,2)) # (Nt * L, in_dim)
            x = x.view(Nt, L, -1)
        out = self.net(x) # (Nt, L, out_dim)
        f1 = (unitx + out[:, :, :self.in_dim*2]).reshape(Nt, *self.f1_size) # (Nt, L*4, 1)
...
for ep in range(1, epochs+1):
    ...
    if ep % 10 == 0:
        ...
        if val_loss_total < best_val_loss:
            ...
            # torch.save(checkpoint, r"../model/CGKN_L1024_stage1.pt")
            status = "✅"
        else:
            status = ""
...
# np.savez(r"../model/CGKN_L1024_loss_history.npz", **loss_history)
# """
# checkpoint = torch.load("../model/CGKN_L1024_stage1.pt", map_location=device)
...
# np.save(r"../data/CGKN_L1024_xy_unit_OneStepPrediction_stage1.npy", test_u1_pred.to("cpu"))
# np.save(r"../data/CGKN_L1024_psi_OneStepPrediction_stage1.npy", test_u2_pred.to("cpu"))
...
# # torch.save(sigma_hat, "../data/CGKN_L1024_sigma_hat.pt")
# sigma_hat = torch.load("../data/CGKN_L1024_sigma_hat.pt")
...
#                     'val_loss': val_loss_total,
#                 }
#                 torch.save(checkpoint, "../model/CGKN_L1024_stage2.pt")
#                 status = "✅"
#             else:
#                 status = ""
...
#                   f"train_z: {loss_forecast_z:.4f}  ae: {loss_ae:.4f} da: {loss_da:.4f} | "
#                   )
# np.savez(r"../model/CGKN_L1024_loss_history_stage2.npz", **loss_history)
# """

# checkpoint = torch.load("../model/CGKN_L1024_stage2.pt", map_location=device)
# cgkn.load_state_dict(checkpoint['model_state_dict'])
...
# # torch.save(train_mu_pred, "../data/CGKN_L1024_train_mu_pred.pt")
...
# # np.save(r"../data/CGKN_L1024_xy_unit_OneStepPrediction.npy", test_u1_pred.to("cpu"))
# # np.save(r"../data/CGKN_L1024_psi_OneStepPrediction.npy", test_u2_pred.to("cpu"))
...
# # test_u1 = test_u1.to("cpu")
# # test_u2 = test_u2.to("cpu")
# # np.save(r"../data/CGKN_L1024_psi_DA.npy", test_mu_pred.to("cpu"))
...
# np.save(r"../data/CGKN_L1024_psi_DA_std.npy", test_mu_std_pred)
