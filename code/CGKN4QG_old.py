...
# torch.save(cgkn, r"../model/QG(Noisy)_sigmaxy01_CGKN_old_dimz256_stage1.pt")
# np.save(r"../model/QG(Noisy)_sigmaxy01_CGKN_old_dimz256_train_loss_forecast_u1_history_stage1.npy", train_loss_forecast_u1_history)
# np.save(r"../model/QG(Noisy)_sigmaxy01_CGKN_old_dimz256_train_loss_forecast_u2_history_stage1.npy", train_loss_forecast_u2_history)
# np.save(r"../model/QG(Noisy)_sigmaxy01_CGKN_old_dimz256_train_loss_ae_history_stage1.npy", train_loss_ae_history)
# np.save(r"../model/QG(Noisy)_sigmaxy01_CGKN_old_dimz256_train_loss_forecast_z_history_stage1.npy", train_loss_forecast_z_history)

# """
# cgkn = torch.load(r"../model/QG(Noisy)_sigmaxy01_CGKN_old_dimz256_stage1.pt").to(device)
...
# train_u1 = train_u1.to("cpu")
# train_u2 = train_u2.to("cpu")
# np.save(r"../data/CGKN_old_sigmaxy01_xy_unit_Sigma_compare0.npy", sigma_hat.to("cpu"))
...
train_u1 = train_u1.to("cpu")
train_u2 = train_u2.to("cpu")
# np.save(r"../data/CGKN_old_sigmaxy01_xy_unit_Sigma_compare1.npy", sigma_hat.to("cpu"))
...
for itr in range(1, Niters+1):
    ...
    loss_forecast_u1 = nnF.mse_loss(u1_short[1:], u1_pred_short[1:])
    loss_forecast_u2 = nnF.mse_loss(u2_short[1:], u2_pred_short[1:])
...
# torch.save(cgkn, r"../model/QG(Noisy)_sigmaxy01_CGKN_old_dimz256_stage2.pt")
np.save(r"../model/QG(Noisy)_sigmaxy01_CGKN_old1_dimz256_train_loss_forecast_u1_history_stage2.npy", train_loss_forecast_u1_history)
np.save(r"../model/QG(Noisy)_sigmaxy01_CGKN_old1_dimz256_train_loss_forecast_u2_history_stage2.npy", train_loss_forecast_u2_history)
np.save(r"../model/QG(Noisy)_sigmaxy01_CGKN_old1_dimz256_train_loss_ae_history_stage2.npy", train_loss_ae_history)
np.save(r"../model/QG(Noisy)_sigmaxy01_CGKN_old1_dimz256_train_loss_forecast_z_history_stage2.npy", train_loss_forecast_z_history)
np.save(r"../model/QG(Noisy)_sigmaxy01_CGKN_old1_dimz256_train_loss_da_history_stage2.npy", train_loss_da_history)
...
# with torch.no_grad():
#     test_z_concat = cgkn.autoencoder.encoder(test_u2).view(Ntest, dim_z)
#     test_u_extended = torch.cat([test_u1, test_z_concat], dim=-1)
#     test_u_extended_pred = cgkn(test_u_extended)
#     test_u1_pred = test_u_extended_pred[:, :dim_u1]
#     test_z_pred = test_u_extended_pred[:, dim_u1:].view(Ntest, int(dim_z**0.5), int(dim_z**0.5))
...
# test_u2 = test_u2.to(device)
# with torch.no_grad():
#     test_mu_z_concat_pred = CGFilter(cgkn, sigma_hat.to(device), test_u1.unsqueeze(-1), mu0=torch.zeros(dim_z, 1).to(device), R0=0.01*torch.eye(dim_z).to(device))[0].squeeze(-1)
#     test_mu_z_pred = test_mu_z_concat_pred.reshape(-1, int(dim_z**0.5), int(dim_z**0.5))
