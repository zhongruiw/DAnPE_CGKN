...
Nt = 405*1E3;      % Number of time steps
qlim = 1E4;  % if any q > qlim, simulation stops
s_rate = 16;   % subsampling rate
...
% Diagnosticse
tstart = Nt-400000;
tstart_fine = Nt-100000;
countDiag = 5000; % Compute diagnostics every countDiag steps
...
psi_1_t_fine = zeros(N-cut*2-1,N-cut*2-1,(Nt-tstart_fine));
    ...
    if ii > tstart
        if mod((ii-tstart),s_rate)==0
            ...
            if ii >= tstart_fine
                ...
                temp1_(cutRow,:) = [];
                temp1_(:,cutRow) = [];
                psi_1_t_fine(:,:,(ii-tstart_fine)) = temp1_;
            end
        elseif ii > tstart_fine
            % Invert for psi
            q_bt = .5*(q(:,:,1) + q(:,:,2));
            q_bc = .5*(q(:,:,1) - q(:,:,2));
            psi_bt = InvBT.*(q_bt - 0.5*hk(:,:)); 
...
if any(isnan(q(:)))
    ...
    save('../../data/qg/QG_DATA_topo32_nu1e-12_beta22_K128_dt1e-3_subs.mat','ii','countDiag','dt','tol','params','T','ke','ape','ene','etp','vb','utz', 'qp','topo','psi_1_t','psi_2_t','s_rate','cut','psi_1_t_fine','-v7.3'); 
end
...
print(h5, 'upper_topo32_nu1e-12_beta22_K128_dt1e-3_snap.png', '-dpng', '-r150') 
...
print(h6, 'lower_topo32_nu1e-12_beta22_K128_dt1e-3_snap.png', '-dpng', '-r150') 
...
xlabel('x'); ylabel('y');
print(h, 'mode_topo32_nu1e-12_beta22_K128_dt1e-3.png', '-dpng', '-r150') 

h1 = figure(2);
set(h1, 'Position', [20, 20, 500, 300]); % Set the figure size ([left, bottom, width, height])
subplot(1,2,1)
loglog([0:N/2],mean(ke(:,end-80:end),2),'.-', 'LineWidth',1); hold on;
title('kinetic energy spectrum'); xlabel('wavenumber');
subplot(1,2,2)
loglog([0:N/2],mean(ape(:,end-80:end),2),'.-', 'LineWidth',1); hold on;
title('potential energy spectrum'); xlabel('wavenumber');
print(h1, 'energy_topo32_nu1e-12_beta22_K128_dt1e-3.png', '-dpng', '-r150')

h3 = figure(3);
set(h3, 'Position', [20, 20, 500, 300]); % Set the figure size ([left, bottom, width, height])
subplot(1,2,1)
loglog([0:N/2],mean(ene(:,end-80:end),2),'.-', 'LineWidth',1); hold on;
title('energy spectrum'); xlabel('wavenumber');
subplot(1,2,2)
loglog([0:N/2],mean(etp(:,end-80:end),2),'.-', 'LineWidth',1); hold on;
title('enstrophy spectrum'); xlabel('wavenumber');
print(h3, 'mode_ene_ens_topo32_nu1e-12_beta22_K128_dt1e-3.png', '-dpng', '-r150')
...
print(h4, 'series_ene_ens_topo32_nu1e-12_beta22_K128_dt1e-3.png', '-dpng', '-r150')